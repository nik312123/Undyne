...
public class StartScreen {
	...
	private static BufferedImage crack;
	...
	public StartScreen() {
		if (Runner.isFirstTime) {
			...
			try {
				...
				// crack =
				// ImageIO.read(Runner.class.getResource("/cracks.png"));
	...
	public void drawSubtitle(Graphics g) {
		if (heartsActivated())
			g.drawImage(subtitleBlue, 0 - scale / 2, 0 - scale / 2, subtitle.getWidth() + scale,
					subtitle.getHeight() + scale, null);
		else
			g.drawImage(subtitle, 0 - scale / 2, 0 - scale / 2, subtitle.getWidth() + scale,
					subtitle.getHeight() + scale, null);
	}
		...
		if (!heartsActivated()) {
			...
			if (!easyButtonRectRed && !hardButtonRectRed && !survivalButtonRectRed) {
				...
				if (warningCounter == 0)
					...
					g2d.drawImage(keys[0], 0 - 150, -220, (int) (1.5 * keys[0].getWidth()),
							(int) (1.5 * keys[0].getHeight()), null);
				}
			} else {
				if (warningCounter == 0)
					...
					g2d.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 1.0f));
					g2d.drawImage(start, 0 - 150, -175, (int) (1.5 * start.getWidth()), (int) (1.5 * start.getHeight()),
							null);
				}
				g2d.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, opacity));
				g2d.drawImage(keys[0], 0, 50 - 20, null);
	...
	public void survivalButton(Graphics g) {
		...
		if (!survivalButtonRectRed)
			g.fillRect(80 - 37 + 148 + 39, 360 - (300 - (Math.abs(300 - survivalButtonRect))) + shift, 140,
					survivalButtonRect);
		else {
	...
	public void drawSpears(Graphics g) {
		if (activateSpears) {
			...
			if (spearFrame <= 30) {
				...
				spearLocation = new Point2D.Double(spearLocation.getX() + (SPEAR_END.getX() - SPEAR_SPAWN.getX()) / 20,
						spearLocation.getY() + (SPEAR_END.getY() - SPEAR_SPAWN.getY()) / 20);
				g2d.drawImage(spear, (int) Math.round(spearLocation.getX()), (int) Math.round(spearLocation.getY()),
						null);
			} else {
				if (!spearHitPlayed) {
	...
	public boolean shouldStart() {
		return !isOnLink() && !isOnHelp() && !heartsActivated() && (hardButtonRectRed && !isOnEasy && !isOnSurvival
				|| easyButtonRectRed && !isOnHard && !isOnSurvival || survivalButtonRectRed && !isOnHard && !isOnEasy);
	}
	...
	public boolean isOnLink() {
		return (heartX + 288 + 16 >= 76 && heartX + 288 <= 224 && heartY + 300 <= 442 + 20
				&& heartY + 300 + 16 >= 380 + 20 && !heartsActivated());
	}

	public boolean isOnHelp() {
		return (heartX + 288 + 16 >= 376 && heartX + 288 <= 524 && heartY + 300 <= 442 + 20
				&& heartY + 300 + 16 >= 380 + 20 && !heartsActivated());
	}
